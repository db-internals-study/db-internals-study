...
## 캐시 비인지형 B-트리
블록 크기, 노드 크기, 캐시 라인 얼라인먼트와 같은 여러 설정 파라미터들은 **B-트리의 성능**에 영향을 미친다.
**캐시 비인지형(cache-oblivious) 자료 구조**는 **메모리 계층 구조**나 **파라미터** 조정 여부와 상관없이 **점근적으로 최적의 성능**을 보장한다.
즉, 알고리즘이 캐시 라인, 파일시스템 블록, 디스크 페이지의 크기 등과 무관하다.

지금까지 B-트리를 **2-레벨 메모리 계층 구조** 관점에서 살펴봤다.
B-트리의 노드는 **디스크의 페이지**에 저장되며 **메인 메모리**에 존재하는 **페이지 캐시**를 활용해 효율적으로 접근한다.
즉, 2레벨 계층 구조는 페이지 캐시와 디스크로 구성된다.

두 개의 레벨로 구성된다는 것은 각 레벨과 관련된 세부사항을 처리하는 레벨 별 코드 모듈이 두 개만 있으면 된다는 것이다. 덕분에 알고리즘 설계가 비교적 수월하다.

디스크는 여러 블록으로 나뉘고, 데이터는 디스크와 캐시 사이에서 블록 단위로 움직인다.
따라서 알고리즘이 블록 한 개의 레코드만 요청해도 블록 전체를 읽어야만 한다.
이러한 방식이 **캐시 인지형(cache-aware) 방식**이다.

성능을 중시하면 CPU 캐시나 디스크 계층 구조를 고려해야만 한다. 그러나 이러한 과정을 일반화하는 것은 어렵다.

**캐시 비인지형 알고리즘**은 **멀티 레벨 계층 구조**의 이점을 제공하면서 자료 구조를 **2-레벨 메모리 계층** 구조의 관점에서 바라볼 수 있게 해준다.
이러한 방식은 플랫폼에 종속적인 파라미터가 필요 없으며 레벨 사이의 전송 횟수를 상수 범위 내로 보장한다.

즉, 자료 구조가 어느 두 레벨의 메모리 계층에서 최적의 성능을 보장하도록 최적화됐다면 인접하는 다른 두 레벨에서도 최적으로 작동한다.
가능한 한 높은 캐시 레벨을 많이 사용할수록 많이 최적화된다.

### 반 엠데 보아스 레이아웃
캐시 비인지형 B-트리는 정적 B-트리와 패킹된(packed) 배열로 구성된다.
정적 B-트리는 반 엠데 보아스(vEB, van Emde Boas) 레이아웃을 기반으로 생성한다.

트리의 에지를 잘라 분할하고, 재귀적으로 각 서브트리를 분할하면 `sqr(N)` 크기의 여러 서브트리가 생긴다.
이 레이아웃의 핵심은 모든 재귀적 트리가 **연속된 메모리 블록**에 저장된다는 것이다.

![](files/6-8%20vEB%20레이아웃.png)
논리적을 그룹화된 노드는 서로 모여있다.

자료 구조를 동적으로 제어하기 위해서 캐시 비인지형 트리는 **패킹된 배열 자료 구조**를 사용한다.
연속된 메모리 세그먼트를 이용해 원소들을 저장하지만, 나중에 삽입될 원소들을 위한 갭(gap)을 마련해둔다.
갭의 간격은 밀도 기준 값에 따라 설정한다.

![](files/6-9%20패킹된%20배열.png)
이 방식은 더 적은 재배치 횟수로 트리에 요소를 삽입할 수 있다.
만약 새로운 요소가 들어갈 갭이 없을 경우 다른 요소를 재배치해야 한다
패킹된 배열의 밀집도가 너무 높거나 너무 낮은 경우 크기를 줄이거나 확장하기 위해서 배열을 재구성해야 한다.

정적 트리는 최하위 레벨의 패킹된 배열의 인덱스로 사용한다.
재배치된 요소가 최하위 레벨의 해당 요소를 가리키도록 업데이트해야 한다.

이런 구조를 사용하면 디스크 기반 자료 구조를 메인 메모리 기반 자료 구조와 유사한 방식으로 만들 수 있다. 하지만 학계 이외에 사용 사례는 아직 없다.

사용 사례가 없는 이유는 캐시 로딩 단계를 추상화해도 데이터를 블록 단위로 읽고 쓰는 구조에서 페이징하고 만료하는 방식이 여전히 결과에 안 좋은 영향을 주기 때문이다.
또 다른 이유는 블록 전송 관점에서 캐시 비인지형 B-트리와 캐시 인지형 B-트리의 복잡도가 같기 때문이다.

효율적인 비휘발성 바이트 단위 스토리지 기기가 보급화되면 바뀌게 될 수도 있다.
